    ...
    >
      <div className="container mx-auto flex items-center justify-between h-20">
        ...
        <div className="hidden md:flex items-center gap-2">
          {navLinks.slice(0, -1).map(link => {
            ...
            return (
              <Link
                ...
                style={{
                  color: active ? 'hsl(var(--amber-light))' : 'white',
                }}
              >
                {link.label}
                {active && (
          ...
          <Link
            to="/contactez-nous"
            className={`ml-4 px-6 py-2.5 rounded-full text-[12px] font-bold uppercase tracking-widest transition-all duration-300 ${scrolled
            ...
            <div className="container py-6 flex flex-col gap-1">
              {navLinks.map((link, i) => (
                ...
                >
                  <Link
                    to={link.path}
                    className={`block px-4 py-3 rounded-lg text-sm font-semibold uppercase tracking-wide transition-colors ${location.pathname === link.path
